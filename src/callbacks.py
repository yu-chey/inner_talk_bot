...
async def start_session_handler(callback: CallbackQuery, state: FSMContext, users_collection) -> None:
    ...
    alert_message = (
        ...
        "Не завершайте сессию, если планируете в ближайшее время продолжить диалог.\n"
        "После диалога, не забывайте завершать сессию, чтобы Ваш психолог обновил данные о вас "
        "и мог в дальнейшем оказать более качественную и индвидуальную поддержку! ❤️"
    )
...
async def get_portrait_handler(callback: CallbackQuery, users_collection, generate_content_sync_func, gemini_client,
    ...
    alert_message = (
        ...
        "Эффективность и точность психологического портрета напрямую зависит от количества сообщений за все сессии 🌟."
    )
